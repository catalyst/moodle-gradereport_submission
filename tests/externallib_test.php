...
namespace gradereport_submission;

use externallib_advanced_testcase;
use gradereport_submission_external;
...
class externallib_test extends externallib_advanced_testcase {
    ...
    public function test_get_grades_table_teacher() {
        ...
        $studentgrades = gradereport_submission_external::get_grades_table($course->id);
        $studentgrades = \external_api::clean_returnvalue(gradereport_submission_external::get_grades_table_returns(),
            $studentgrades);
    ...
    public function test_get_grades_table_student() {
        ...
        $studentgrade = gradereport_submission_external::get_grades_table($course->id, $student1->id);
        $studentgrade = \external_api::clean_returnvalue(gradereport_submission_external::get_grades_table_returns(),
            $studentgrade);
    ...
    public function test_view_grade_report() {
        ...
        $this->setUser($student1);
        $result = gradereport_submission_external::view_grade_report($course->id);
        $result = \external_api::clean_returnvalue(gradereport_submission_external::view_grade_report_returns(), $result);
        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);

        // Check the event details are correct.
        $this->assertInstanceOf('\gradereport_submission\event\grade_report_viewed', $event);
        $this->assertEquals(\context_course::instance($course->id), $event->get_context());
        $this->assertEquals($USER->id, $event->get_data()['relateduserid']);

        $this->setUser($teacher);
        $result = gradereport_submission_external::view_grade_report($course->id, $student1->id);
        $result = \external_api::clean_returnvalue(gradereport_submission_external::view_grade_report_returns(), $result);
        $events = $sink->get_events();
        $event = reset($events);
        $sink->close();
        ...
        $this->setUser($student2);
        try {
            $studentgrade = gradereport_submission_external::view_grade_report($course->id, $student1->id);
    ...
    public function test_get_grade_items_student() {
        ...
        $studentgrades = gradereport_submission_external::get_grade_items($course->id, $student1->id);
        $studentgrades = \external_api::clean_returnvalue(gradereport_submission_external::get_grade_items_returns(),
            $studentgrades);
        // No warnings returned.
        $this->assertCount(0, $studentgrades['warnings']);
